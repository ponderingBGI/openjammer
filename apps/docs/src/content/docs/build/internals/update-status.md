...
|---|---|---|
| **Windows** | ✅ Enabled | Installs the signed NSIS `.exe` update. Until an Authenticode identity is provisioned, first-run may show a SmartScreen "unknown publisher" prompt — a one-time OS trust step, separate from minisign payload signing. |
