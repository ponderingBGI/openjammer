...
- **Phase 3.4** — **every connection port in the node system is now oj-ui `Port`; zero legacy port classes remain.** Every schematic / standard / panel / bundle / visual-dive-in port → `Port`/`PortRow`/`KeyTile` (incl. the rainbow-universal treatment taught to `Port`, and the panel empty-slot → `Port placeholder`); in-node controls → `Slider`/`Button`/`Select`/`ValueScrubber`; the control-active highlight is a real `Port active` state (`.oj-port.is-active`). Reaper #9a–#9e removed **every** dead legacy node class — `.port-dot`/`.node-btn`/`.port*` twins, 21 faceplate port-markers, `.port-circle-marker`/`.control-active`, 46 dead node-content classes, and finally the panel/bundle markers + the orphan-token bundle colors + the standalone `.output-port` + the orphaned `rainbow-shift` keyframe (`SchematicNodes.css` 4138 → 2830 lines; `BundlePortGroup.css` orphan tokens for ports gone). The canvas hit-test union is now purely node-roots + `.oj-port`/`.oj-port-row`. No deleted class is referenced by any CSS or tsx. Method: quote-agnostic "appears-nowhere" scan + parallel-Opus hunt-then-adversarial-refute workflow (KEEP-on-doubt) + brace-aware parser (drop a rule only when every comma-segment involves a dead class) + diff review + live smoke.
  - **Sanctioned app-side exception** (not a gap): `MiniLab3Visual` keeps its device-specific `minilab3-key-port` status dots — the whole key/pad/knob is the click target and the dot is a passive per-device-layout connection indicator, which doesn't map to oj-ui `Port` (the interactive port). Plan §1.3 keeps this device faceplate app-side. Its remaining `--hover-bg`/`--scrollbar-thumb`/`--text-*` orphan-token *chrome* fallbacks (shared with other chrome) are part of the broader chrome tokenization = Phase 4 designer work.

- **Phase 3.5 / 3.6 — chrome leaf rewire DONE** (user-approved *normalization*: the look converges on oj-ui Button; small metric shifts accepted, canonical metrics deferred to the designer). Toolbar (icon buttons + File/Edit/View menu triggers), Settings (panel/keybindings/audio/updates), Guides, MIDI, Clips, Breadcrumbs, BundlePortGroup-expand, CommandBar (cmdk-safe — palette/composer/custom-nav left raw), Collab → oj-ui `Button`/`Select`/`Input`/`Textarea`; boolean checkboxes → `Toggle`, the zoom range → `Slider`. Dead bespoke button CSS reaped (~640 lines across the chrome CSS). oj-ui `Button` gained `forwardRef` (AiPanel/DropdownMenu need a button ref). Semantic fix vs the agents' first pass: the success-GREEN `active` prop was reverted for "selected"/"customized" indicators (settings tabs, keybindings-custom) back to their real neutral/accent className highlight. Each surface live-verified in-browser (Settings tabs neutral not green; cmdk filter intact; menus open; toggle switches). Deliberate non-migrations: panic-safe `AppErrorBoundary`/`KeybindingsErrorBoundary`; cmdk command-items + the chat composer + custom-keyboard-nav listboxes; the bespoke stateful widgets `collab-toggle` (peer badge) / `back-to-action`; the hidden file input.

**REMAINING — human/designer-gated only:**
- **`SchematicNodes.css` + chrome literal-color/animation tokenization** — choosing each color's semantic role affects all themes; Phase 4 designer work, not a mechanical no-op.
- **Phases 4–6** (below): the new default theme, Figma Variables/Tokens Studio sync, Code Connect (needs Figma node IDs), and the `claude.ai/design` `/design-sync` run (external publish + auth — **user-triggered**; staged + ready now to seed the designer). Designer arrives ~2026-06-22.
