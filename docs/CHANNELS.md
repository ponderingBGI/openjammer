...
3. **The asset path.** `AssetPcm` keeps a mono fast-path but may retain `n_channels` so a *stereo
   Sampler* (a node with a 2-channel output port) plays a stereo sample. Downmix stays available for a
   mono destination via the §4 rule.
