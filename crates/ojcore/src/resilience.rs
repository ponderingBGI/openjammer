...
impl NodeBudget {
    ...
    pub fn with_nodes(n: usize) -> Self {
        Self { non_finite: vec![false; n], over_budget: vec![false; n] }
    }
...
impl Watchdog {
    ...
    pub fn new(budget_ns: u128, auto_bypass: bool) -> Self {
        Self { budget_ns, started: None, auto_bypass }
    }
    ...
    pub fn from_block(sample_rate: f32, block_size: usize, fraction: f32) -> Self {
        let sr = if sample_rate > 0.0 { sample_rate as f64 } else { 48_000.0 };
...
impl CommandCoalescer {
    ...
    pub fn with_capacity(cap: usize) -> Self {
        Self { params: Vec::with_capacity(cap), other: Vec::with_capacity(cap) }
    }
        ...
        match cmd {
            RtCommand::SetParam { node, param, value } => {
                for existing in &mut self.params {
                    if let RtCommand::SetParam { node: n, param: p, value: v } = existing {
                        if *n == node && *p == param {
...
mod tests {
    ...
    fn coalescer_last_wins_for_same_param() {
        ...
        c.push(RtCommand::SetParam { node: NodeIdx(1), param: 0, value: 0.1 });
        c.push(RtCommand::SetParam { node: NodeIdx(1), param: 0, value: 0.2 });
        c.push(RtCommand::SetParam { node: NodeIdx(1), param: 0, value: 0.3 });
    ...
    fn coalescer_keeps_distinct_params_and_passthrough() {
        let mut c = CommandCoalescer::with_capacity(8);
        c.push(RtCommand::SetParam { node: NodeIdx(1), param: 0, value: 0.5 });
        c.push(RtCommand::SetParam { node: NodeIdx(2), param: 0, value: 0.6 }); // diff node
        c.push(RtCommand::SetParam { node: NodeIdx(1), param: 1, value: 0.7 }); // diff param
        c.push(RtCommand::Bypass { node: NodeIdx(1), on: true }); // pass-through
        c.push(RtCommand::SetParam { node: NodeIdx(1), param: 0, value: 0.9 }); // coalesces w/ first
        ...
        assert!(applied.iter().any(|c| matches!(c, RtCommand::Bypass { on: true, .. })));
    }
